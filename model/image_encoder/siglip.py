...
class M3DCLIPConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        siglip_loss: bool = True,  # ← Toggle added
        in_channels: int = 1,
        ...
        **kwargs,
        ...
        self.siglip_loss = siglip_loss  # ← Toggle registered
...
class M3DCLIP(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.siglip_loss = config.siglip_loss
    ...
    def forward(self, images, input_ids, attention_mask, labels=None, **kwargs):
        ...
        logits = self.logit_scale * all_image_features @ all_text_features.T  # shape [B, B]

        if self.siglip_loss:
            # Sigmoid-based BCE loss (SigLIP)
            batch_size = logits.shape[0]
            labels_matrix = torch.eye(batch_size, device=logits.device)
            loss_fct = nn.BCEWithLogitsLoss()
            loss = loss_fct(logits, labels_matrix)
        else:
            # Standard CLIP-style symmetric softmax cross-entropy loss
...
# Register model with HuggingFace registry
